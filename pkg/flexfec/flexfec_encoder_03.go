...
func (flex *FlexEncoder03) encodeFlexFecHeader(mediaPackets *util.MediaPacketIterator, mask1 uint16, optionalMask2 uint32, optionalMask3 uint64, mediaBaseSn uint16) []byte {
	fmt.Println()
	fmt.Println("ENCODING A HEADER")
	fmt.Println("media packet SSRC = ", mediaPackets.First().SSRC)
	fmt.Println()
	fmt.Println("Base SN for batch of media packets = ", mediaBaseSn, " = %b", mediaBaseSn)
	fmt.Println()
	...
	*/
	...
	for mediaPackets.HasNext() {
		...
		fmt.Println("Xoring a media packet's header...")
		fmt.Println("	sequence number => ", mediaPacket.SequenceNumber)
		fmt.Println("	timestamp => ", mediaPacket.Timestamp, " = %b", mediaPacket.Timestamp)
